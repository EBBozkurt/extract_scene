...
def read_from_excel(path : str, sheetname : str, cell : str):
    wb = load_workbook(path, data_only=True)
    sh = wb[sheetname]
    return sh[cell].value

def add_to_excel(path: str, sheetname : str, cell : str, value : str):
    wb = load_workbook(path, data_only=True)
    sh = wb[sheetname]
    sh[cell].value = value
    wb.save(path)

def get_row_count_from_excel(path : str,sheetname : str):
    wb = load_workbook(path, data_only=True)
    sh = wb[sheetname]
    return sh.max_row

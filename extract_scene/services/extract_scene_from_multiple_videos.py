import os
from .extract_scene import extract_scenes2,increase_video_count,get_video_name
...
class ExtractScenesFMV:
    ...
    def extract_scenes_fmv(self): # Extract scene from multiple videos in the provided dir.
        ...
        for file in self.directory:
            ...
            # Call the function from the extract_scene file to increase the video number for the next video naming
    ...
    def add_to_excel(self):
        for video in get_video_name():
            self.export.add_data_to_table(video) # Add every video name in the list to the excel table

        print(f"{len(get_video_name())} row added to excel.") # Inform the user

        get_video_name().clear() # Clear the list for every video for the memory efficiency
